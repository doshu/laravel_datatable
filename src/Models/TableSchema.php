<?php
    ...
    namespace Doshu\LaravelDatatable\Models;
    ...
    abstract class TableSchema
    {
        ...
        public function __construct($backendAction, $request) {
        ...
        public function getConfig() {
            ...
            return [
                ...
                'backend_action' => $this->_backendAction,
        ...
        public function prepareCollection($collection) {
            $queryParams = $this->_request->query();
            ...
            if(!empty($queryParams['sort'])) {
                $sortIndex = $this->getColumnSortIndex($queryParams['sort']);
                $collection->orderBy($sortIndex, $queryParams['direction'] ?? 'asc');
            }
            
            //pagination
            $collection = $collection->paginate($queryParams['limit'] ?? 20);
            ...
            foreach($this->getColumns() as $column => $columnData) {
                ...
                foreach($data as &$row) {
                    $row['columns'][$column] = $formatter(\Illuminate\Support\Arr::get($row['original'], $index), $row);
                }
            }
            ...
            return [
                ...
                'data' => [
                    ...
                    'pagination' => [
                        ...
                        'end' => $collection->lastItem(),
                        'limit' => $collection->perPage(),
                        'page' => $collection->currentPage(),
                        'pageCount' => $collection->lastPage(),
                        'perPage' => $collection->perPage(),
                        'requestedPage' => $collection->currentPage(),
                        'sort' => $queryParams['sort'] ?? null,
                        'start' =>  $collection->firstItem(),
                    ]
                ]
            ];
        }
        ...
        public function _formatCurrency($value, $column, $row, $options = []) {
            return (new \NumberFormatter(app()->currentLocale(), \NumberFormatter::CURRENCY))->formatCurrency($value, 'EUR');
        }
        ...
        public function _formatElement($value, $column, $row, $options = []) {
            $renderer = $this->getColumnRenderer($column);
            $html = view($renderer, compact('value', 'column', 'row', 'options') + ['table' => $this]);
            return $html;
        }
        ...
        protected function _getFilterApplier($column) {
            ...
            $filterClass = "\\Doshu\\LaravelDatatable\\Models\\Filter\\".$filterClass;
